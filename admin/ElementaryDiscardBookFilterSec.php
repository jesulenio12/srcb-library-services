<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE discarded = '1' && remove = '0' && libraryClass = 'Grade School Library' && bookSection != 'Periodical' && bookSection LIKE '$bookSection%' ORDER BY `id` DESC") or die(mysqli_error());
...
?>
	<tr>
		...
		<td align="center" class="noprint">
				<form method="POST" action="admin.php?action=ElementaryDiscardedBookCard&&id=<?php echo $fetch['id']; ?>" style="display:inline">
					<button class="btnd-1" type="submit">
						<i class="glyphicon glyphicon-bookmark"></i>
					</button>
				</form>
				<form method="POST" action="admin.php?action=ElementaryDiscardedBookEdit&&id=<?php echo $fetch['id']; ?>" style="display:inline">
					<button class="btnd-1" type="submit">
				...
				<form method="POST" action="gs_remove-discardedbooks.php" style="display:inline">
...
<?php
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE discarded = '1' && remove = '0' && libraryClass = 'Grade School Library' && bookSection != 'Periodical' ORDER BY `id` DESC") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td>#</td>
