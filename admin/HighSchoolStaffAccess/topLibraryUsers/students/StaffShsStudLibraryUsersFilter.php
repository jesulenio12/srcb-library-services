...
if (isset($_POST['filter'])) {
    ...
    $sql = "SELECT
            ...
            WHERE 
                ...
                AND
                userType = 'Student' && libraryClass = 'High School Library' AND MONTH(timeIn) = MONTH(CURRENT_DATE) AND YEAR(timeIn) = YEAR(CURRENT_DATE)
            GROUP BY
    ...
    $sql = "SELECT
            ...
            FROM
                attendance
            WHERE
                libraryClass = 'High School Library' && userType = 'Student' AND MONTH(timeIn) = MONTH(CURRENT_DATE) AND YEAR(timeIn) = YEAR(CURRENT_DATE)
            GROUP BY
                attendance.library_userID, attendance.fullname, attendance.gender, attendance.yearLevel, attendance.progtrack, attendance.classSection, attendance.departmentType
            ORDER BY
                EntryCount DESC";
