<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `booktransactions` WHERE transactionType = 'return' && remarks = 'Overdue' && transactionPlace = 'Hs-Lib' && userType LIKE '$userType%' AND gender LIKE '$gender%' AND yearLevel LIKE '$yearLevel%' AND classSection LIKE '$classSection%' AND progtrack LIKE '$progtrack%' AND bookSection LIKE '$bookSection%'") or die(mysqli_error());
...
<?php
		...
		$dueDate = 0;
		$totalfines = 0;
		$query=mysqli_query($conn, "SELECT * FROM `booktransactions` WHERE transactionType = 'return' && remarks = 'Overdue' && transactionPlace = 'Hs-Lib'") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td>#</td>
