...
if (isset($_POST['filter'])) {
    ...
    $sql = "SELECT
            ...
            FROM
                booktransactions
            WHERE 
                transactionDate BETWEEN '$firstDate' AND '$secondDate' AND gender LIKE '$gender%' AND departmentType LIKE '$departmentType%'
                AND
                userType = 'Teacher' && transactionType = 'return' && transactionPlace = 'Col-Lib' AND MONTH(transactionDate) = MONTH(CURRENT_DATE) AND YEAR(transactionDate) = YEAR(CURRENT_DATE)
            GROUP BY
                booktransactions.library_userID, booktransactions.fullname, booktransactions.gender, booktransactions.departmentType
            ORDER BY
                EntryCount DESC";
