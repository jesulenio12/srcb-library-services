...
if (isset($_POST['filter'])) {
    ...
    $sql = "SELECT
            ...
            WHERE 
                ...
                AND
                userType = 'Teacher' && libraryClass = 'College Library' AND MONTH(timeIn) = MONTH(CURRENT_DATE) AND YEAR(timeIn) = YEAR(CURRENT_DATE)
            GROUP BY
    ...
    $sql = "SELECT
            ...
            FROM
                attendance
            WHERE
                libraryClass = 'College Library' && userType = 'Teacher' AND MONTH(timeIn) = MONTH(CURRENT_DATE) AND YEAR(timeIn) = YEAR(CURRENT_DATE)
            GROUP BY
                attendance.library_userID, attendance.fullname, attendance.gender, attendance.departmentType
            ORDER BY
                EntryCount DESC";
