<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `booktransactions` WHERE payment = 0 && departmentType = 'Higher Education Department' && userType LIKE '$userType%' AND gender LIKE '$gender%' AND yearLevel LIKE '$yearLevel%' AND program LIKE '$program%' AND bookSection LIKE '$bookSection%'") or die(mysqli_error());
...
<?php
		...
		$dueDate = 0;
		$totalfines = 0;
		$query=mysqli_query($conn, "SELECT * FROM `booktransactions` WHERE transactionType = 'return' && payment = 0 && departmentType = 'Higher Education Department'") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td>#</td>
