<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' && lost = 1 && remove = 1 && discarded = 0 && bookSection LIKE '$bookSection%' AND libraryClass LIKE '$libraryClass%' ORDER BY `id` DESC") or die(mysqli_error());
...
<?php
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' && lost = 1 && remove = 1 && discarded = 0 ORDER BY `id` DESC") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td>#</td>
