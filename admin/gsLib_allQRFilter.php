...
if (isset($_POST['filterbook'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' AND lost='0' AND discarded='0' AND libraryClass = 'Grade School Library' " . ($whereClause ? " AND $whereClause" : "") . " ORDER BY `id` DESC") or die(mysqli_error());
					...
					<div class="card">
						<div class="card-body">
							<div class="card-label-button">
								...
								<form method="post" action="hs_move_library_book.php">
							...
							<img src="admin/gsBookQRCodes/<?php echo $fetch['qrcode']?>" width="170px" height="140px" class="card-img-qr">
							<br><br>
    ...
    $query = mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' AND lost='0' AND discarded='0' AND libraryClass = 'Grade School Library' ORDER BY `id` DESC") or die(mysqli_error());
			...
			<div class="col-md-4" style="margin:10px 10px 10px 10px; padding-left:40px;" class="column">
				...
				<div class="card">
					<div class="card-body">
						<div class="card-label-button">
							<span class="dateFilter"><?php echo date('m-d-Y', strtotime($fetch['dateQRfilter'])); ?></span>
							<form method="post" action="gs_move_library_book.php">
								<input type="hidden" name="bookAccession" value="<?php echo $fetch['bookAccession']; ?>">
								<button type="submit" class="btn btn-primary mt-2"><i class="fa fa-arrow-right"></i></button>
							</form>
						</div>
