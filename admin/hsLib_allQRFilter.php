...
if (isset($_POST['filterbook'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' AND lost='0' AND discarded='0' AND libraryClass = 'High School Library' " . ($whereClause ? " AND $whereClause" : "") . " ORDER BY `id` DESC") or die(mysqli_error());
    ...
    $query = mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' AND lost='0' AND discarded='0' AND libraryClass = 'High School Library' ORDER BY `id` DESC") or die(mysqli_error());
