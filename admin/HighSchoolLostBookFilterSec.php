<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' && lost = 1 && remove = 0 && discarded = 0 && libraryClass = 'High School Library' && bookSection LIKE '$bookSection%' ORDER BY `id` DESC") or die(mysqli_error());
...
?>
	<tr>
		...
		<td align="center" class="noprint">
			<form method="POST" action="admin.php?action=HighSchoolLostBookCard&&id=<?php echo $fetch['id']; ?>" style="display:inline">
...
<?php
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'Periodical' && lost = 1 && remove = 0 && discarded = 0 && libraryClass = 'High School Library' ORDER BY `id` DESC") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td>#</td>
