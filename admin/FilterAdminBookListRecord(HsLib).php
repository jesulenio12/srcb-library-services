<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'General References' AND bookSection != 'Dictionaries' AND bookSection != 'Periodicals' && status='0' && discarded='0' && libraryClass = 'High School Library' && bookSection LIKE '$bookSection%' ORDER BY `id` ASC") or die(mysqli_error());
...
?>
	<tr>
		...
		<td align="center" class="noprint">
			...
			<form method="POST" action="admin.php?action=AdminHighSchoolBookCard&&id=<?php echo $fetch['id']; ?>" style="display:inline">
...
<?php
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE bookSection != 'General References' AND bookSection != 'Dictionaries' AND bookSection != 'Periodicals' && status='0' && discarded='0' && libraryClass = 'High School Library' ORDER BY `id` ASC") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td><?php echo $fetch['bookAccession']?></td>
