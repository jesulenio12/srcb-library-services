<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $sql = "SELECT bookTitle, count(*) AS topBookname
                FROM booktransactions
                WHERE transactionType = 'return' && transactionPlace = 'Col-Lib'
                AND transactionDate BETWEEN '$startDate' AND '$endDate' AND bookSection LIKE '$bookSection%' AND departmentType LIKE '$departmentType%'
                GROUP BY bookTitle ORDER BY topBookname DESC LIMIT 5";

        $sql2 = "SELECT bookSection, count(*) AS topBooksection
                FROM booktransactions
                WHERE transactionType = 'return' && transactionPlace = 'Col-Lib'
                AND transactionDate BETWEEN '$startDate' AND '$endDate' AND bookSection LIKE '$bookSection%' AND departmentType LIKE '$departmentType%'
                GROUP BY bookSection ORDER BY topBooksection DESC LIMIT 5";
    ...
    while($row2 = mysqli_fetch_array($result2))
    {
        ...
        $test2[$count2]["y"] = $row2["topBooksection"];
        $count2 = $count2 + 1;
    }
                                ...
                                <div class="col-md-3"></div>
                                    ...
                                    <center>
                                        ...
                                        <div class="under-label-btn">
                                            <a style="color: #fff;" href="admin.php?action=CollegeMostBorrowedBooks"><button type="button" style="background: #fff; color: #3db166;" class="abtnb">TABLE DATA </button></a>
                                            <a style="color: #fff;" href="admin.php?action=CollegeMostBorrowedBooksGraph"><button type="button" style="background: #3db166; color:white" class="abtnr">GRAPHICAL DATA</button></a> 
                                        </div>
                                    </center>
                                    <center>
                                        <p class="styled-text">HED MOST BORROWED BOOKS RECORD</p>
                                    </center>
                                    <br>
                                    <div class="noprint">
                                            ...
                                            <div class="wrap-filter">
                                                ...
                                                <div class="filter-input-group"> 
                                                    ...
                                                    <select class="filter-input-box" for="bookSec" name="bookSec" id="bookSec">
                                                        ...
                                                        <?php
                                                            $booktransactions = DB:: getInstance()->query("SELECT DISTINCT bookSection FROM booktransactions WHERE libraryClass = 'College Library' ORDER BY id DESC");							
                                                            foreach($booktransactions->results() as $booktransactions){
                                                        ?>
                                                        <option value="<?php echo $booktransactions->bookSection?>"><?php echo ucwords($booktransactions->bookSection) ?></option>
                                                        <?php }?>
                                                    </select> 
                                                </div>
                                                <div class="filter-input-group"> 
                                                ...
                                                <a class="filtersbtn" href="admin.php?action=CollegeMostBorrowedBooksGraph"><button style="background:transparent; border:none; padding-top: 7px; color:#fff" type="button"><i class="fa fa-refresh"></i></button></a>
