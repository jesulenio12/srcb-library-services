...
if (isset($_POST['filter'])) {
    ...
    $sql = "SELECT
                ...
                booktransactions.bookTitle,
				booktransactions.bookSection,
				booktransactions.departmentType,
                COUNT(*) as EntryCount
            FROM
                booktransactions
            WHERE 
                transactionDate BETWEEN '$firstDate' AND '$secondDate' AND gender LIKE '$gender%' AND progtrack LIKE '$progtrack%' AND yearLevel LIKE '$yearLevel%' AND bookSection LIKE '$bookSection%' AND departmentType LIKE '$departmentType%'
                AND
                transactionType = 'return' && transactionPlace = 'Col-Lib' AND MONTH(transactionDate) = MONTH(CURRENT_DATE) AND YEAR(transactionDate) = YEAR(CURRENT_DATE)
            GROUP BY
                booktransactions.library_userID, booktransactions.fullname, booktransactions.gender, booktransactions.progtrack, booktransactions.yearLevel,  booktransactions.bookTitle, booktransactions.bookSection, booktransactions.departmentType
            ORDER BY
                EntryCount DESC";
