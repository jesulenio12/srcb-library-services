<?php
    ...
    $sql = "SELECT fullname, count(*) AS mostLog
            FROM attendance
            WHERE libraryClass = 'College Library' && userType = 'Student' && MONTH(timeIn) = MONTH(CURRENT_DATE) AND YEAR(timeIn) = YEAR(CURRENT_DATE)
    ...
    $sql2 = "SELECT progtrack, count(*) AS mostLog
            FROM attendance
            WHERE libraryClass = 'College Library' && userType = 'Student' && MONTH(timeIn) = MONTH(CURRENT_DATE) AND YEAR(timeIn) = YEAR(CURRENT_DATE)
            GROUP BY progtrack ORDER BY mostLog DESC LIMIT 5";
...
<title>HED Students Library Daily Users</title>
...
<script>
    ...
    var chart = new CanvasJS.Chart("chartContainer2", {
        animationEnabled: true,
        title: {
            text: "TOP DAILY USERS BY PROGRAM"
        },
        subtitles: [{
            // text: "November 2017"
        }],
                ...
                <section class="content-header" style="background-color:transparent">
                  <br>
                    ...
                    <ol class="breadcrumb" style="background-color: rgba(255, 255, 255, 0.239); border-radius:30px; margin-top:-5px;">
                        ...
                        <a href="admin.php?action=HedStudLibraryUsers"><button type="button" class="btn" style="background: #3db166; color:white">Students</button></a> 
                        <a href="admin.php?action=HedTeachLibraryUsers"><button type="button" class="btn" style="color: #3db166;">Teachers</button></a> 
                    </ol>
                    </center>
                </section>
                            ...
                            <div class="box box-primary" style="background-image: url(images/nav2.jfif); background-size:cover; background-position:center center; background-repeat: no-repeat; border-radius: 30px; padding:10px">
                                        ...
                                        <form class="form-inline" method="POST" action="">
                                            <center>
												<div class="wrap-filter" class="noprint">
													...
													<a class="filtersbtn" href="admin.php?action=HedStudLibraryUsers"><button style="background:transparent; border:none; padding-top: 7px; color:#fff" type="button"><i class="fa fa-refresh"></i></button></a>
												</div>
											</center>
                                        </form>
                                    </div>
                                ...
                                <div class="box-body table-responsive">
                                    ...
                                    <table class="table table-bordered table-hover" id="articles">
                                        ...
                                        <tbody>
												<?php include 'HedStudLibraryUsersFilter.php'?>	
										</tbody>
                                    </table>
                                </div><!-- /.box-body -->
                            </div><!-- /.box -->
