...
if (Input::exists()) {
		...
		if ($books->count()){
			...
			Redirect::to('admin.php?action=HS-OnlineBookBorrowed');
			} catch(Exception $e) {
			$error;
			}
...
<title>HS Online Book Borrowed</title>
</head>
                  ...
                  <br>
                    <center>
						<ol class="breadcrumb" style="background-color: rgba(255, 255, 255, 0.239); border-radius:30px; margin-top:-5px;">
							...
							<a href="admin.php?action=HS-OnlineBookRequested"><button type="button" class="btn" style="color: #3db166;">Book Requested</button></a> 
							<a href="admin.php?action=HS-OnlineBookBorrowed"><button type="button" class="btn" style="background: #3db166; color:white">Book Borrowed</button></a>
							<a href="admin.php?action=HS-OnlineBookReturned"><button type="button" class="btn" style="color: #3db166;">Book Returned</button></a>  
						</ol>
                    </center>
                </section>
                <!-- Main content -->
                                    ...
                                    <table class="table table-bordered table-hover" id="books">
                                        ...
                                        <tbody>
											<?php
												...
												$totalfines = 0;
												$dueDate = 0;
												$books = DB:: getInstance()->query("SELECT * FROM books WHERE approved = '1' && received = '0' && libraryClass = 'High School Library'");
												foreach($books->results() as $books){ ?>
														<tr>
															<td>#</td>
															...
															<td align="center">
																<?php if($books->yearLevel == ''){?>
																	---
																<?php }else{?>
																	<?php echo $books->yearLevel?>
																<?php }?>
															</td>
															<td><?php echo $books->progtrack ; ?></td>
															<td align="center">
																<?php if($books->progtrack == ''){?>
																	---
																<?php }else{?>
																	<?php echo $books->progtrack?>
																<?php }?>
															</td>
															<td>
																<?php
																	if($books->departmentType == 'Higher Education Department'){
																		echo 'HED';
																	}elseif($books->departmentType == 'Senior High School Department'){
																		echo 'SHS';
																	}else{
																		echo 'JHS';
																	}
