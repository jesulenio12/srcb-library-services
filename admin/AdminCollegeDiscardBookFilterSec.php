<?php
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE discarded = '1' && status='Available' && bookSection != 'Periodical' && bookSection LIKE '$bookSection%' AND libraryClass LIKE '$libraryClass%' ORDER BY `id` DESC") or die(mysqli_error());
...
?>
	<tr>
		...
		<td align="center" class="noprint">
			<a style="color:white" href="admin.php?action=AdminDiscardedBookCard&&id=<?php echo $fetch['id']; ?>"><button type="button" class="catalog"><i class="glyphicon glyphicon-bookmark"></i> Card Catalog </button></a> 
		</td>
	</tr>
<?php
			}
		...
		$query=mysqli_query($conn, "SELECT * FROM `books` WHERE discarded = '1' && status='Available' && bookSection != 'Periodical' ORDER BY `id` DESC") or die(mysqli_error());
		while($fetch=mysqli_fetch_array($query)){
?>
	<tr>
		<td>#</td>
