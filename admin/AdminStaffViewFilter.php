...
if (isset($_GET['library_userID'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM userlogin WHERE library_userID = '$library_userID' AND archive = 0 AND permission != 5 AND permission != 6 ORDER BY `id` ASC;
	") or die(mysqli_error());
		...
		<div class="avatar-row">
			<div class="col-md-6" class="column">
				<div class="card" style=" background-image: url('admin/images/background.jpg'); background-size: cover; background-position: center;">
					...
					<div class="card-body">
							<h2 class="user-type"><?php echo getRoleByPermission($fetch['permissionRole']);?></h2>
						<p>
							...
							Date Added: <?php echo date('d F Y | h:i A', strtotime($fetch['dateQRfilter']));?>
						</p>
					</div>
				</div>
			</div>
		...
		<div class="avatar-row">
			<div class="col-md-6">
				...
				<div class="secondary-info-card">
					<a style="color: #fff;" href="admin.php?action=userList">
					...
					<?php
						if ($fetch['permission'] != 1) {
							echo '
								<form method="POST" action="" style="display:inline">
									<input type="hidden" name="delete" value="' . urlencode($fetch['id']) . '">
							...
							require_once('delete-confirm.php');
						}
					?>
				</div>
			</div>
