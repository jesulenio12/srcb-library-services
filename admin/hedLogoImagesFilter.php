<?php
require 'conn.php';
require_once 'core/init.php';

    $query = mysqli_query($conn, "SELECT * FROM `srcblogo` WHERE logoClass = 'College Library' ORDER BY `id` DESC") or die(mysqli_error($conn));
	$row = mysqli_num_rows($query);

    if ($row > 0) {
        while ($fetch = mysqli_fetch_array($query)) {
            ?>
           	<div class="row mt-5">
				<div class="col-md-4" style="margin:10px 10px 10px 10px; padding-left:40px;" class="column">
					<div class="card">
						<div class="card-body">
							<div class="card-label-button">
								<?php require_once ('delete-confirm.php');?>
								<form method="post" action="hed_delete_logo_image.php">
									<input type="hidden" name="logoImageId" value="<?php echo $fetch['id']; ?>">
									<button type="button" data-toggle="modal" data-target="#confirmDelete" data-title="Confirm Delete" class="btn btn-primary mt-2"><i class="glyphicon glyphicon-remove"></i></button>
								</form>
							</div>
							<img src="admin/hedLogoImages/<?php echo $fetch['logoImage']?>" width="200px" height="200px" class="card-img-qr">
							<br><br><br>
							<p class="card-text" style="margin-top:-25px">
								<span style="font-size: 15px; font-weight: bolder;">
									<?php echo $fetch['logoName']?>
								</span>
							</p>
						</div>
					</div>
				</div>
			</div>
            <?php
        }
    }


?>
