...
if (isset($_POST['filterbook'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `bookimages` WHERE bookImageLibClass = 'College Library' " . ($whereClause ? " AND $whereClause" : "") . " ORDER BY `id` DESC") or die(mysqli_error($conn));
					...
					<div class="card">
						<div class="card-body">
							<div class="card-label-button">
								...
								<form method="post" action="hed_delete_book_image.php">
							...
							<img src="admin/hedBookImages/<?php echo $fetch['bookImage']?>" width="250px" height="350px" class="card-img-qr">
							<br><br><br>
    ...
    $query = mysqli_query($conn, "SELECT * FROM `bookimages` WHERE bookImageLibClass = 'College Library' ORDER BY `id` DESC") or die(mysqli_error($conn));
	$row = mysqli_num_rows($query);
