...
require 'conn.php';
    ...
    $query = mysqli_query($conn, "SELECT * FROM `indivqrcard` WHERE archive = 0 && libraryClass = 'High School Library' && departmentType = 'Senior High School Department' && userType = 'Student' ORDER BY `id` DESC") or die(mysqli_error());
    while ($fetch = mysqli_fetch_array($query)) {
        ?>
        <div class="row mt-5">
            <div class="col-md-4" style="margin:10px 10px 10px 10px;" class="column">
                <div class="card-label-button">
					<form method="post" action="shs_return_library_card.php">
