...
if (Input::exists()) {
		...
		if ($books->count()){
			...
			Redirect::to('admin.php?action=HS-OnlineBookReturned');
			} catch(Exception $e) {
			$error;
			}
...
<title>HS Online Book Returned</title>
</head>
                  ...
                  <br>
                    <center>
						<ol class="breadcrumb" style="background-color: rgba(255, 255, 255, 0.239); border-radius:30px; margin-top:-5px;">
							...
							<a href="admin.php?action=HS-OnlineBookBorrowed"><button type="button" class="btn" style="color: #3db166;">Book Borrowed</button></a>
							<a href="admin.php?action=HS-OnlineBookReturned"><button type="button" class="btn" style="background: #3db166; color:white">Book Returned</button></a>  
						</ol>
                    </center>
                </section>
                <!-- Main content -->
                                    ...
                                    <table class="table table-bordered table-hover" id="books">
                                        ...
                                        <tbody>
											<?php
												...
												$totalfines = 0;
												$dueDate = 0;
												$books = DB:: getInstance()->query("SELECT * FROM books WHERE received = '1' && libraryClass = 'High School Library'");
												foreach($books->results() as $books){ ?>
														<tr>
															<td>#</td>
