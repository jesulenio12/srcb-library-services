...
if (isset($_POST['filterbook'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `bookimages` WHERE bookImageLibClass = 'Grade School Library' " . ($whereClause ? " AND $whereClause" : "") . " ORDER BY `id` DESC") or die(mysqli_error($conn));
					...
					<div class="card">
						<div class="card-body">
							<div class="card-label-button">
								...
								<form method="post" action="gs_delete_book_image.php">
							...
							<img src="admin/gsBookImages/<?php echo $fetch['bookImage']?>" width="250px" height="350px" class="card-img-qr">
							<br><br><br>
    ...
    $query = mysqli_query($conn, "SELECT * FROM `bookimages` WHERE bookImageLibClass = 'Grade School Library' ORDER BY `id` DESC") or die(mysqli_error($conn));
	$row = mysqli_num_rows($query);
