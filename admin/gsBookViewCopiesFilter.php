...
if (isset($_GET['bookTitle'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM books WHERE bookTitle = '$bookTitle' AND bookSection != 'Periodical' AND lost = 0 AND discarded = 0 AND libraryClass = 'Grade School Library'") or die(mysqli_error());
    ...
    while ($fetch = mysqli_fetch_array($query)) {
        ...
        <tr>
            ...
            <td align="center" class="noprint">
                <form method="POST" action="admin.php?action=ElementaryBookCard&bookTitle=<?php echo urlencode($_GET['bookTitle']); ?>&id=<?php echo $fetch['id']; ?>" style="display:inline">
					<button class="bookbtn1" type="submit">
						<i class="glyphicon glyphicon-bookmark"></i>
					</button>
				</form>
				<form method="POST" action="admin.php?action=ElementaryBookEdit&bookTitle=<?php echo urlencode($_GET['bookTitle']); ?>&id=<?php echo $fetch['id']; ?>" style="display:inline">
					<button class="bookbtn1" type="submit">
