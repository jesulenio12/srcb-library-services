...
if (isset($_POST['search'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `userlogin` WHERE archive = 0 && libraryClass = 'High School Library' && departmentType = 'Senior High School Department' && userType = 'Student' " . ($whereClause ? " AND $whereClause" : "") . " ORDER BY `id` DESC") or die(mysqli_error());
            ...
            <div class="row mt-5">
                <div class="col-md-4" style="margin:10px 10px 10px 10px;" class="column">
                    <div class="card-label-button">
                        ...
                        <form method="post" action="shs_move_library_card.php">
                    ...
                    <div class="card" style="background:white; border: 5px solid <?php echo getColorByProgTrack($fetch['progtrack']); ?>; border-radius:10px; width:443px;">
                        ...
                        <img src="admin/shsStudQRCodes/<?php echo $fetch['qrcode'] ?>" width="200px" height="200px" style="border-radius:10px; margin-left:10px; margin-right:-10px;" class="card-img-qr">
                        <!-- Add a delete button -->
                    </div>
                </div>
            </div>
    ...
    $query = mysqli_query($conn, "SELECT * FROM `userlogin` WHERE archive = 0 && libraryClass = 'High School Library' && departmentType = 'Senior High School Department' && userType = 'Student' ORDER BY `id` DESC") or die(mysqli_error());
