...
if (isset($_POST['search'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `userlogin` WHERE archive = 0 && libraryClass = 'College Library' && departmentType = 'Higher Education Department' && userType = 'Teacher' " . ($whereClause ? " AND $whereClause" : "") . " ORDER BY `id` DESC") or die(mysqli_error());
            ...
            <div class="row mt-5">
                <div class="col-md-4" style="margin:10px 10px 10px 10px;" class="column">
                    <div class="card-label-button">
                        ...
                        <form method="post" action="hedTeach_move_library_card.php">
                    ...
                    <div class="card" style="background:white; border: 5px solid; border-radius:10px; width:443px;">
                        ...
                        <img src="admin/hedTeachQRCodes/<?php echo $fetch['qrcode'] ?>" width="200px" height="200px" style="border-radius:10px; margin-left:10px; margin-right:-10px;" class="card-img-qr">
                        <!-- Add a delete button -->
                    </div>
                </div>
            </div>
    ...
    $query = mysqli_query($conn, "SELECT * FROM `userlogin` WHERE archive = 0 && libraryClass = 'College Library' && departmentType = 'Higher Education Department' && userType = 'Teacher' ORDER BY `id` DESC") or die(mysqli_error());
