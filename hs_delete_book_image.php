...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['bookImageId'])) {
        ...
        if ($deleteQuery) {
            ...
            // Make sure to replace 'admin/hsBookImages/' with your actual path.
            $filePath = 'admin/hsBookImages/' . $id; // Adjust this to match your file structure
            if (file_exists($filePath)) {
                unlink($filePath);
            }
            Session::flash('Deleted', "Book image has been deleted successfully.");
            Redirect::to('admin.php?action=hsLibBookImages');    
        } else {
